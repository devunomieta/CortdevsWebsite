                        ...
                        return (
                            <div key={t.id} className="flex items-center justify-between px-5 py-4 gap-4">
                                <div className="min-w-0">
                                    <p className="font-medium text-sm truncate">{t.user_email || t.user_id}</p>
                                    <p className="text-xs text-muted-foreground truncate">
                                        {KIND_LABEL[t.kind] || t.kind}{t.listing_title ? ` · ${t.listing_title}` : ""}{t.service_name ? ` (${t.service_name})` : ""}{t.source ? ` · ${t.source}` : ""} · {new Date(t.created_at).toLocaleString()}
                                    </p>
                                    {t.reference && <p className="text-[10px] text-muted-foreground font-mono truncate">{t.reference}</p>}
                                </div>
